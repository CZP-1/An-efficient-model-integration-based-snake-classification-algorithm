...
def validate(args):
    ...
    data_val = pd.read_csv("new_val.csv",header=0)
    ...
    report = classification_report(cat_preds, cat_labels)
    print(report)
    df_R = pd.DataFrame([report])
    df_R.to_csv('classification_report.csv')

    C = confusion_matrix(cat_preds, cat_labels)
    df_C = pd.DataFrame(C)
    df_C.to_csv('confusion_matrix.csv')
